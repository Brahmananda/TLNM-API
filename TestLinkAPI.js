...
/**
 * Get files Attached Created for TestCase.
 * 	@param 	 testCaseExternalId
 * 	@param 	 downloadPath
 */
TestLinkApi.prototype.getTestCaseAttachments = function(params,downloadpath,callback,supportedFiles) {
    var post = utilites.postCompose(this.url),
		inputObject = {
        ...
        testcaseexternalid: {
            ...
            type: "string"
					...
					var filename=component_names[index];
					index++;	
					var matchFlag=true;
					if(supportedFiles && Array.isArray(supportedFiles))
					{
							matchFlag = supportedFiles.some(function (element, index, array) {
							var sFileExtension = filename.split('.')[filename.split('.').length - 1];
							return (element == sFileExtension);
							});
			        }
			
			if(matchFlag)
			{
            fs.writeFile(downloadpath+filename, buf, function(err) {
                if (err) throw err;
 ...
 * 	@param 	uploadPath
 */
TestLinkApi.prototype.uploadTestCaseAttachment = function(params,uploadPath,callback) {
    var post = utilites.postCompose(this.url),
